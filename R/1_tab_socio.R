...
gdp <- as_tibble(gdp[get_state(gdp$name), ])
saveRDS(gdp, "data/tab/gdp.rds")

pop <- read_sidra("data/sidra/pop.ods")
pop <- as_tibble(pop[get_state(pop$name), ])


# Add census for 2007 and 2010 --------------------------------------------

pop07 <- read_sidra("data/sidra/pop2007.ods")
pop07 <- as_tibble(pop07[get_state(pop07$name), ])

# 2010 contains info on urban/rural and indigenous population
