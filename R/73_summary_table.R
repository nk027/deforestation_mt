...
data_summary <- data %>% 
  ...
  summarise_at(vars(forest_ch_km2, forest_px_km2_lag, pasture_px_km2_lag,
                    crop_px_km2_lag, pop_km2_lag_log, cattle_dens_lag_log, 
                    soy_filled_lag, spei_dry), 
...
row.names(out) <- c("Forest change", "Forest share", "Pasture share", 
  "Cropland share", "Population density", "Cattle density",
  "Soy yield", "Dry month")
knitr::kable(round(out, 3), format = "latex", booktabs = TRUE)
