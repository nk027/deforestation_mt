...
stopifnot(
  file.exists("data/data_raw.rds"),
  require(dplyr),
  require(sf),
  nzchar(system.file(package = "lattice")),
  nzchar(system.file(package = "reshape2")),
  nzchar(system.file(package = "ggplot2"))
)

data <- shp <- readRDS("data/data_raw.rds")
data$geometry <- NULL
...
get_W <- function(x) {
  ...
  nb <- sf::as_Spatial(dplyr::filter(x, date == 2015))
...
# Get code for offending municipios
worst_muni <- municipios %>% 
  filter(soy_brl_hha > 12) %>% .$code
bad_muni <- municipios %>% 
  filter(soy_brl_hha > 5 & soy_brl_hha < 13) %>% .$code
ok_muni <- municipios %>% 
  filter(soy_brl_hha < 6 & soy_brl_hha > 0) %>% .$code
...
data <- data %>% 
  mutate(soy_filled = ifelse(is.na(soy_brl_hha), 
                             ifelse(is.na(soy_filled_1), 
                                    soy_filled_2, soy_filled_1), soy_brl_hha))


# 3, worst fills - treat Amazon and others separately

# Specifically look at the Amazon
...
ggplot2::ggplot(x) +
  ggplot2::geom_line(ggplot2::aes(x = date, y = soy_filled, colour = code)) +
  ggplot2::geom_line(data = data.frame(x = 2002:2017, y = yearly), 
                     ggplot2::aes(x, y))
...
# Add filled values to the sf object
shp <- merge(shp, data[, c("code", "date", "soy_filled")], 
